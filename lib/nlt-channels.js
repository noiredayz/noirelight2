...
class TChannel{
	constructor (name, chmode, context, bpapi, sr, monitor, chid, links) {
		this.name = name;
		this.chmode = chmode;
	...
	if((!chr.chid || chr.chid===0) && chr.context==="twitch"){
		...
		nlt.got.get("https://customapi.aidenwallis.co.uk/api/v1/twitch/toID/"+chr.name).then((data) => {
			if(!isNaN(data.body)){
				...
				nlt.channels.push(new TChannel(chr.name, chr.chmode, chr.context, chr.bpapi_url, chr.sr, chr.monitor, data.body, chr.links));
				return 2;
			} else {
				...
				nlt.channels.push(new TChannel(chr.name, chr.chmode, chr.context, chr.bpapi_url, chr.sr, chr.monitor, chr.chid, chr.links));
				return 1;
			}
		});
	} else {
