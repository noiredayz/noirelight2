...
class TChannel{
	constructor (name, chmode, context, bpapi, sr, monitor, chid, links, monitorbans) {
		this.name = name;
		this.chmode = chmode;
...
function LoadSingleChannel(chr){
	if((!chr.chid || chr.chid===0) && chr.context==="twitch"){
		nlt.util.printtolog(LOG_INFO, `<loadch> ${chr.name} has no twitch ID set, attempting to get it from API.`);
		nlt.got.get("https://customapi.aidenwallis.co.uk/api/v1/twitch/toID/"+chr.name).then((data) => {
			if(!isNaN(data.body)){
				nlt.util.printtolog(LOG_INFO, `<loadch> Successfully updated the channel id of channel ${chr.name} to ${data.body}`);
				nlt.maindb.insertQuery(`UPDATE channels
										SET chid='${data.body}'
										WHERE name='${chr.name}' AND context='twitch';`);
				nlt.channels.push(new TChannel(chr.name, chr.chmode, chr.context, chr.bpapi_url, chr.sr, chr.monitor, data.body, chr.links, chr.monitorbans));
				return 2;
			} else {
				nlt.utl.printtolog(LOG_INFO, `<loadch> warning: Aiden's API returned NaN for chid query, not changing it. Return value was ${data.body}`);
				nlt.channels.push(new TChannel(chr.name, chr.chmode, chr.context, chr.bpapi_url, chr.sr, chr.monitor, chr.chid, chr.links, chr.monitorbans));
				return 1;
			}
		});
	} else {
