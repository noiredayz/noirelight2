...
class TChannel{
	constructor (name, chmode, context, bpapi, sr, monitor, chid, links, monitorbans, pb2, offlineOnly, webLog) {
		this.name = name;
		this.chmode = chmode;
	...
	if((!chr.chid || chr.chid===0) && chr.context==="twitch"){
		...
		helixGetData("users", "login="+chr.name).then((data) => {
			if(data){
				...
				nlt.channels.push(new TChannel(chr.name, chr.chmode, chr.context, chr.bpapi_url, chr.sr, chr.monitor, data[0].id, chr.links, chr.monitorbans, chr.pajbot2, chr.offlineOnly, chr.weblog));
				return 2;
			} else {
				...
				nlt.channels.push(new TChannel(chr.name, chr.chmode, chr.context, chr.bpapi_url, chr.sr, chr.monitor, chr.chid, chr.links, chr.monitorbans, chr.pajbot2, chr.offlineOnly, chr.weblog));
				return 1;
			}
		}).catch((err)=>{
