"use strict";
const {printtolog, getRndInteger, floatToPercentText, stringCheck} = require(process.cwd()+"/lib/nlt-tools.js");
...
function hxGetUserID(unick){
return new Promise(async (resolve, reject) => {
	if(!stringCheck(unick)){
		reject(new TypeError("helix getuserid: parameter must be a string"));
	}
	if(unick.length<3 || unick.length>25){
		reject("parameter is not a valid twitch username(too long or too short)");
	}
	let rrows = nlt.maindb.selectQuery(`SELECT * FROM twitch_users WHERE nick='${unick.toLowerCase()}';`);
	if(rrows.length>0){
		resolve(rrows[0].uid);
		return;
	}
