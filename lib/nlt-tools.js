...
function timeDelta(intime, useSecs=false){
	if(useSecs){							//if set the intime is assumed to be unix time in seconds
		return(getunixtime()-Number(intime));
	} else {								//else its unix time in ms, like what new Date returns Okayeg
		return ((new Date)-Number(intime));
	}
}
...
function stringCheck(inVar, compStr=undefined){
	if(!inVar) return false;
	if(typeof(inVar)!="string") return false;
	if(inVar.length===0) return false;
	if(compStr && !stringCheck(compStr)) return false;		//I hecking love recursion BatChest
	if(!compStr)
		return true; //lulw
	else
		return (inVar===compStr);
}
...
function validateTwitchUsername(unick){
	...
	if(!stringCheck(unick)) return false;
	if(unick.length<3 || unick.length>25) return false;
	const utest = /^[a-zA-Z0-9][\w]{2,24}$/; // credits to https://discuss.dev.twitch.tv/u/Freddy
...
const knownBots = new Set ([
  ...
  'enlightenbot',     'deepdankdungeonbot',
...
exports.validateTwitchUsername = validateTwitchUsername;
