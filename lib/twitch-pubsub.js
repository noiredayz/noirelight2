...
const {printtolog, getRndInteger} = require("./nlt-tools.js");
const {LOG_NO, LOG_DBG, LOG_INFO, LOG_WARN} = require("./nlt-const.js");
...
async function joinChannels(){
	...
	if(!nlt.cache.getD(helix_bearer)){
		try {
			await(nlt.ss["twitch"].ttvAuthenticate());
		}
		catch(err){
			printtolog(LOG_WARN, `<pubsub> Twitch subsystem couldn't acquire a helix bearer. Cannot continue.`);
			return;
		}
	}
	...
	if (rrows.length===0){
		...
		nlt.util.printtolog(LOG_DBG, `<pubsub> initializing ${rrows.length} channels for start/stop events`);
	...
	case 'MESSAGE':
		if (msg.data) {
			...
			case 'viewcount':
				...
				break;
			case 'commercial':
				printtolog(LOG_INFO, `<pubsub> A channel is running a commercial... uBlock gang WideHard`);
				break;
			case 'stream-up':
			case 'stream-down':
				...
				break;;
			default:
				printtolog(LOG_WARN, `<pubsub> unknown topic message type: [${msgTopic}] ${JSON.stringify(msgData)}`);
			}
		} else {
			printtolog(LOG_WARN, `<pubsub> no data associated with message [${JSON.stringify(msg)}]`);
		}
		break;
	case 'RECONNECT':
		printtolog(LOG_WARN, '<pubsub> Pubsub server sent a reconnect message. restarting the socket');
		ps.reconnect();
		break;
	default:
		printtolog(LOG_WARN, `<pubsub> received unknown pubsub message type: ${msg.type}`);
	}
});
...
async function listenStreamStatus (target_channel) {
	...
	const message = {
		...
		'data': {
			...
			'auth_token': nlt.cache.getd("helix_bearer")
		},
	};
...
async function handleWSMsg (msg = {}) {
	...
	if (msg) {
		...
		case 'reward-redeemed':
			//TODO: NaM
			printtolog(LOG_INFO, `<pubsub> Channelpoint redemption on channel ${target_channel} by ${msg.data.user.display_name}, item: ${msg.data.reward.title}, optional text if any: ${msg.data.usr_input}`);
			break;
		case 'stream-up':
			...
			printtolog(LOG_WARN, `<pubsub> ${target_channel} is online`);
		...
		case 'stream-down':
			...
			printtolog(LOG_WARN, `<pubsub> ${target_channel} is offline`);
			rrows = await psdb.PselectQuery(`SELECT * FROM ps WHERE name='${target_channel}';`);
...
function handleWSResp (msg) {
	if (!msg.nonce) {
		printtolog(LOG_WARN, `<pubsub> Unknown message without nonce: ${JSON.stringify(msg)}`);
		return;
	}
	let rrows = psdb.selectQuery(`SELECT * FROM ps WHERE nonce='${msg.nonce}';`);
	if (msg.error) {
		printtolog(LOG_WARN, `<pubsub> Error occurred while subscribing to topic ${rrows[0].topic} for channel ${rrows[0].name}: ${msg.error}`);
	} else {
		printtolog(LOG_WARN, `<pubsub> Successfully subscribed to topic ${rrows[0].topic} for channel ${rrows[0].name}`);
	}
};
